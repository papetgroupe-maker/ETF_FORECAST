...
// Route API serverless (Next.js) : génère P10/P50/P90 via OpenAI si la clé existe,
// sinon fallback local (drift + volatilité lognormale).
...
// Appel OpenAI (JSON strict) — nécessite OPENAI_API_KEY dans Vercel
...
export async function GET(req: Request, { params }: { params: { ticker: string } }) {
  ...
  // 1) Historique synthétique (90 jours ouvrés) — juste pour le graphe (cours réel non requis ici)
  const muDaily = 0.0003;
  const sigDaily = 0.012;
  const histDates = bizBack(new Date(), 90);

  // RNG déterministe par ticker pour stabilité visuelle
  let seed = Array.from(ticker).reduce((a, c) => a + c.charCodeAt(0), 0) >>> 0;
  const rnd = () => ((seed = (1664525 * seed + 1013904223) >>> 0) / 0xffffffff);

  let price = 500; // point de départ mock
  ...
  // 2) Prévisions : tente OpenAI si la clé est présente, sinon fallback local
  ...
  try {
    if (process.env.OPENAI_API_KEY) {
      ...
      // Validation longueur
      if (
        out?.p10?.length === futDates.length &&
        out?.p50?.length === futDates.length &&
        out?.p90?.length === futDates.length
      ) {
        p10 = out.p10.map((x) => Number(x));
        p50 = out.p50.map((x) => Number(x));
        p90 = out.p90.map((x) => Number(x));
